...
1. **Option resolution**
   ...
   - Context (`ctx`) stores derived booleans such as `either`, `normalizeSoftBreaks`, and `considerInlineBoundaries` (true only when `normalizeSoftBreaks` is false), plus cached punctuation data (`punctuationSpace`, `punctuationConfig`). `punctuationConfig` carries `maxLength`, an `endCharMap` for fast last-character short-circuit checks, and pre-sorted `lengths` to avoid unnecessary suffix scans.
2. **Rule registration**
   - `cjk_breaks_plugin` guards against missing `md.core.ruler`, installs at most one core rule named `cjk_breaks` per `MarkdownIt` instance, and uses first-install-wins semantics for repeated `.use(...)` calls on the same instance. All heavy lifting happens during this pass over inline tokens.
